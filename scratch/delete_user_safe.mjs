...
const USER_ID = 'f85f3f8f-61bd-4b9b-adbf-ca2c1f7699b4'; // karunyajothiprakash811@gmail.com
...
async function safeCleanTable(table, column, val, isDelete = false) {
  try {
    ...
    if (error) {
      // If error is code 42P01 (table doesn't exist) or similar, ignore it
...
async function run() {
  console.log(`🚀 Starting safe cleanup for User ID: ${USER_ID} (karunyajothiprakash811@gmail.com)...`);

  // List of tables/columns to nullify
  const columnsToNull = [
  ...
  // List of tables to hard delete rows from
